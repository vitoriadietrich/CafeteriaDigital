...
Projeto desenvolvido para o desafio "Explorando Padrões de Projeto na Prática" (Digital Innovation One). Em vez de apenas reproduzir os exemplos originais, criei um domínio próprio — uma cafeteria digital — para aplicar os padrões de forma mais próxima de um cenário real de portfólio, combinando os três padrões trabalhados em aula com dois padrões adicionais.
...
O Main.java simula 3 pedidos diferentes (um espresso simples pago no Pix, um cappuccino com leite e chantilly pago no cartão, e um espresso com leite pago em dinheiro), mostrando no console:
...
Referências
DIO — Padrões de Projeto com Java Puro
